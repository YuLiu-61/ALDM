...
def generate_binomial_mask(B, T, N,p=0.5):
    return torch.from_numpy(np.random.binomial(1, p, size=(B, T,N))).to(torch.bool)

class TSEncoder(nn.Module):
    def __init__(self, input_dims, output_dims, hidden_dims=128, depth=10, mask_mode='binomial'):
        super().__init__()
    ...
    def forward(self, x, mask=None):  # x: B x T x input_dims
        ...
        if T<60:
            ...
            if mask == 'binomial':
                ...
                mask = x.new_full((x.size(0), x.size(1),x.size(2)), False, dtype=torch.bool)
            elif mask == 'mask_last':
                mask = x.new_full((x.size(0), x.size(1),x.size(2)), True, dtype=torch.bool)
                mask[:, -1] = False
            # mask &= nan_mask
...
class TSEncoder_all_repr(nn.Module):
    ...
    def forward(self, x, mask=None):  # x: B x T x input_dims
        ...
        x[~nan_mask] = 0
        x = self.input_fc(x)  # B x T x Ch,Input Projection Layer
        
        # generate & apply mask(Timestamp Masking)
        if mask is None:
            if self.training:
                mask = self.mask_mode
            else:
