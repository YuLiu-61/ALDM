...
class DeepSVDDTrainer:
    ...
    def train(self, train_loader, test_loader, net, args):
        logger = log()
        self.args = args
        # Set device for network
        ...
        # Set optimizer (Adam optimizer for now)
        optimizer = optim.Adam(net.parameters(), lr=0.0001, weight_decay=args.weight_decay)
        ...
        for epoch in range(20):
            ...
            gt, pre = self.test(net, test_loader, 0,1, self.device)
            logger.print_result(gt, pre, (net, self.c), args.seed, args)
    ...
    def test(self, net, test_loader, delta_t, sigma, device):
            ...
            logdata = pd.DataFrame(loss)
            logdata['label'] = test_loader.dataset.label
            logdata.to_csv('svdd.csv')
            # auc_score, fps,tps = roc_auc_all(loss, delta_t, sigma)
            # auc_score = roc_auc_score(np.asarray(test_loader.dataset.label,dtype=int),loss)
            ...
            return np.asarray(test_loader.dataset.label,dtype=int), loss
...
class DeepSVDD(object):
    ...
    def train(self, train_loader, test_loader, args, device: str = 'cuda'):
        """Trains the Deep SVDD model on the training data."""
       
        self.trainer = DeepSVDDTrainer(device=device)
        # Get the model
        self.trainer.train(train_loader, test_loader, self.net, args)
...
class DeepSADTrainer:
    ...
    def train(self, train_loader, test_loader, net, args):
        ...
        for epoch in range(20):
            ...
            for data, semi_targets, idx in train_loader:
                ## for unsupervised setting
                # ran1 = torch.bernoulli(0.3*torch.ones_like(semi_targets))
                # semi_targets = (ran1.int() | semi_targets.int())
                # ran0 = torch.bernoulli(0.7*torch.ones_like(semi_targets))
                # semi_targets = (ran0.int() & semi_targets.int()).long()
                ## for occ setting
                semi_targets
                x = data.to(self.device)
                x = torch.transpose(x, dim0=2, dim1=3)
