...
class MADE(nn.Module):
    def __init__(self, input_size, hidden_size, n_hidden, cond_label_size=None, activation='relu', input_order='sequential', input_degrees=None):
        ...
        # construct model
        self.net_input = MaskedLinear(input_size, hidden_size, masks[0], cond_label_size)#cond_label_size=hidden_size
        self.net = []
        for m in masks[1:-1]:
...
class MAF(nn.Module):
    ...
    def base_dist(self, z, k, window_size):
        # C = z.shape[-1]
        # C = 51
        N =  z.shape[0]//k//window_size#batch_size
        p=self.base_dist_mean.repeat_interleave(window_size, 0)
        logp = - 0.5*(z-p.repeat(N,1))**2
    ...
    def log_prob(self, x, k, window_size, y=None):
        ...
        C = u.shape[1]
        return torch.sum(self.base_dist(u, k, window_size) + sum_log_abs_det_jacobians, dim=1)+ C * _GCONST_
