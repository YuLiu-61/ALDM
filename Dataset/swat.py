...
def loader_SWat(root, batch_size, window_size, stride_size,train_split,label=False):
    ...
    try:
        ...
        # Handle mixed formats or format inconsistencies
        Timestamp = pd.to_datetime(data["Timestamp"], errors='coerce', dayfirst=True)
...
class SWat_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        time = self.timeindex[index].strftime('%Y-%m-%d %H:%M:%S')
